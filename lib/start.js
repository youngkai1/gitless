...
const pushOrigin = ()=>[
  ...
  'git pull',
  'git add *',
  `git commit -m "${ cmdArgs.pushRemark||'update' }"`,
  'git push'
]
const addTag = ()=>{
  const now = new Date()
...
class GitLess {
  constructor(parameters) {
    if(args.length==0){
      this.readInput("选择快捷操作模式:\nlpush:提交代码; atag:提交新标签; dtag:删除标签; \n", chunk=>{
  ...
  deltag(){
    if( args[1] ){
      ...
      this.showAlltag( tags=>{
        this.readInput("请输入需删除的序号或标签名\n", chuank=>{
          cmdArgs.deltag = tags[chuank]||tags.find(item=>item==chuank)
  ...
  showAlltag(callback){
    exec("git for-each-ref --sort=taggerdate --format '%(refname)' refs/tags", ( err, stdout, stderr ) => {
      if(err){
        return console.log(stderr)
      }
      if(stdout){
        console.log("已存在的标签列表")
      }
      let tags = stdout.replace(/(\'|\")+/g,"").split("\n").map( (item, index)=>{
